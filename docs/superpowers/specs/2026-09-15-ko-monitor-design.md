...
- Karakter ölüyken (`hp == 0`) açık olan tanınmayan pencere ayrıca bildirilmez (Ölü bildirimi zaten gitti); ölüyken okunan `disconnect_dialog` da disconnect okuması sayılmaz. Pencere yazısındaki disconnect ifadeleri tam kelime/ifade olarak, harfler küçültülüp Türkçe karakterler sadeleştirilerek (ı→i, ğ→g, ş→s, ç→c, ö→o, ü→u) aranır; böylece pencerenin arkasındaki isim yazıları kelime içi eşleşmeyle yanlış disconnect sayılmaz.
